...
async function registerLayerZeroEndpoint(chainId: number) {
  const LZEndpointMock = (await (
    await (await ethers.getContractFactory("LZEndpointMock")).deploy(chainId)
...
async function registerErc721Mock(
  ...
  const ERC721Mock = (await (
    await (await ethers.getContractFactory("ERC721Mock")).deploy(name, symbol)
...
async function registerLNONFT(
  ...
  const lnONFT = await (
    ...
    ).deploy(lzEndpointAddr, baseUri, minGas, royalitiesReceiver.address, owner)
  ).deployed();
