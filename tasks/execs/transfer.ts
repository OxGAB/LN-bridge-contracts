...
export const transferLN__task = async (
    args: {
        start?: string;
        end?: string;
        to?: string;
    ...
    if (
        ...
        throw new Error('Long Necks can only be on the Canto network');
    }
    ...
    for (let i = start; i < end; i++) {
        const tx = await longNecks.transfer(to, i);
        await tx.wait();
        console.log(`Transfered Long Neck #${i} to ${to}`);
    }
};
